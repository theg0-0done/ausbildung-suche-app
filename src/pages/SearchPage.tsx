...
export default function SearchPage({ onSelectJob }: SearchPageProps) {
  ...
  const {
    ...
    getNextPageParam: (lastPage, allPages) => {
      const loadedCount = allPages.reduce(
        (acc, page) => acc + (page.stellenangebote?.length || 0),
        0,
      );
  ...
  useEffect(() => {
    if (error) {
      showNotification(
        (error as Error).message || "Die Suche ist fehlgeschlagen.",
        "error",
      );
    }
  ...
  const results = data
    ? data.pages.flatMap((p) => p.stellenangebote || [])
    : [];
  const total = data?.pages[0]?.maxErgebnisse || 0;
  const isLoading = isFetching && !isFetchingNextPage;

  const handleSearch = useCallback(
    (was: string, wo: string) => {
  ...
  const displayedResults = results.filter((job) => {
    if (filters.ausbildungsart) {
      ...
      const titleMatches = (job.titel || "")
        .toLowerCase()
        .includes(filters.ausbildungsart.toLowerCase());
          ...
          {!isLoading &&
            ...
            !error && (
              <div className="empty-state">
                ...
                <p>
                  Versuche es mit anderen Suchbegriffen oder einem anderen Ort.
                </p>
              </div>
            )}
