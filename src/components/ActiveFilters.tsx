...
const ANGEBOTSART_MAP: Record<string, string> = {
  '1': 'Arbeit',
  '2': 'Selbständigkeit',
...
const VEROEFFENTLICHT_MAP: Record<string, string> = {
  '1': 'Seit 24 Stunden',
  '7': 'Seit 7 Tagen',
  '30': 'Seit 30 Tagen',
};
...
export default function ActiveFilters({ filters, onRemove }: ActiveFiltersProps) {
  ...
  if (filters.umkreis) chips.push({ key: 'umkreis', label: `+ ${filters.umkreis} km` });
  
  if (filters.angebotsart && filters.angebotsart !== '4') {
    chips.push({ key: 'angebotsart', label: ANGEBOTSART_MAP[filters.angebotsart] || 'Angebotsart' });
  }
  ...
  if (filters.veroeffentlichtseit) {
    chips.push({ key: 'veroeffentlichtseit', label: VEROEFFENTLICHT_MAP[filters.veroeffentlichtseit] || 'Datum' });
  }

  if (filters.arbeitszeit && filters.arbeitszeit.length > 0) {
    filters.arbeitszeit.forEach(az => {
      chips.push({ key: 'arbeitszeit', value: az, label: ARBEITSZEIT_MAP[az] || az });
    });
  }
