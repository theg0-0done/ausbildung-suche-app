...
import { sendOtp, verifyOtp as verifyOtpService, isOtpVerified, clearOtp } from '../otpService.js';
  ...
  try {
    ...
    if (!user) {
      const { data: newUser, error: insertError } = await supabase
        .from('users')
        .insert([{ email, password_hash: 'oauth-google', display_name: displayName }])
        .select('id, email, display_name')
        .single();
      
      if (insertError || !newUser) {
        console.error('Database error creating Google user:', insertError);
        res.status(500).json({ error: 'DB Insert Error: ' + insertError?.message });
        return;
      }
      user = newUser;
    }
    ...
    res.status(500).json({ error: 'Google Auth Error: ' + (err.message || String(err)) });
  }
});
...
router.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    const { data: newUser, error: insertError } = await supabase
      .from('users')
      .insert([{ email, password_hash: passwordHash, display_name: displayName || email.split('@')[0] }])
      .select('id, email, display_name')
      .single();
...
router.get('/me', requireAuth, async (req: Request, res: Response) => {
  try {
    const { data: user, error } = await supabase
      .from('users')
      .select('id, email, display_name, preferred_location, preferred_category, created_at')
...
router.post('/send-otp', async (req: Request, res: Response) => {
  try {
    ...
    if (purpose === 'reset') {
      ...
      if (!existing) {
        res.status(404).json({ error: 'Kein Konto mit dieser E-Mail gefunden.' });
        return;
      }
    }
    ...
    if (!result.success) {
      res.status(500).json({ error: result.error || 'OTP konnte nicht gesendet werden.' });
      return;
    }
...
router.post('/verify-otp', (req: Request, res: Response) => {
  const { email, code, purpose } = req.body as { email?: string; code?: string; purpose?: 'register' | 'reset' };

  if (!email || !code || !purpose) {
    res.status(400).json({ error: 'E-Mail, Code und Zweck sind erforderlich.' });
    return;
  }
...
router.post('/reset-password', async (req: Request, res: Response) => {
  try {
    ...
    if (!email || !newPassword) {
      res.status(400).json({ error: 'E-Mail und neues Passwort sind erforderlich.' });
      return;
    }
    ...
    if (!isOtpVerified(email, 'reset')) {
      res.status(403).json({ error: 'Bitte bestätige zuerst deinen E-Mail-Code.' });
      return;
    }
